...
```bash
npm ci
npm test                 # vitest run (47 tests across 3 files)
...
**Test files:**
- `tests/lib/lattice-engine.test.mjs` — estimateTokens, buildLatticeExecution (16 tests)
- `tests/lib/lattice-access.mjs` — normalizeEmail, isValidEmailShape, makeGuestGrant, CREATOR_EMAIL (15 tests)
- `tests/research/holographic-operators.test.mjs` — operatorCoherence, scoreBoundaryBulkBeta, scoreScaleInvariance, scorePhaseLockGamma, scoreEpistemicInversion (16 tests)
...
|----------|---------|-------------|
| `test.yml` | push/PR to lib/, research/**, tests/**, package* | `npm ci && npm test` |
