...
/** Reset stale OpenRouter (or unknown) selections back to Cursor — Bridge owns OpenRouter. */
export function migrateActiveProvider(): LatticeProvider {
  try {
    const raw = String(localStorage.getItem(PROVIDER_STORAGE) || '').trim().toLowerCase();
    if (isLatticeProvider(raw)) return raw;
    if (raw) localStorage.setItem(PROVIDER_STORAGE, 'cursor');
  } catch {
    /* ignore */
  }
  return 'cursor';
