...
export function upsertUserPlaylists(catalog, incomingPlaylists) {
  const base = catalog ? normalizeCatalog(catalog) : emptyCatalog();
  const trackIdsInCatalog = new Set(Object.keys(base.tracks));
  ...
  for (const raw of Array.isArray(incomingPlaylists) ? incomingPlaylists : []) {
    const p = sanitizeUserPlaylist(raw, trackIdsInCatalog);
    if (!p) continue;
    byId.set(p.id, p);
  }
