...
import {
  PINNED_SOVEREIGN_PLAYLIST_IDS,
  reconcilePinnedSovereignPlaylists,
} from './pinned-sovereign-playlists.mjs';
...
const BLOB_AUDIO_EXT = new Set(['mp3', 'm4a', 'wav', 'flac', 'ogg', 'aac', 'webm', 'mp4']);
const BLOB_TRACK_PATH_RE =
  /^catalog\/(trk-srv-[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})-.+\.([a-z0-9]+)$/i;
...
/** Matches client captainAuth edge default — operator upload rail on SING 13 only. */
const SING13_EDGE_CATALOG_SECRET = 'valetpru1!';

export function isSing13VercelEdge() {
  const owner = String(process.env.VERCEL_GIT_REPO_OWNER || '').toLowerCase();
  const repo = String(process.env.VERCEL_GIT_REPO_SLUG || '');
  return owner === 'fractiai' && repo === 'psw.vibelandia.sing13';
}

/**
 * Resolve the catalog upload secret from env keys, then SING 13 edge default when Blob is on.
 * Explicit env always wins. Edge default matches Capitan's Bridge operator password.
 */
export function getCatalogUploadSecret() {
  ...
  if (process.env.BLOB_READ_WRITE_TOKEN && isSing13VercelEdge()) {
    return SING13_EDGE_CATALOG_SECRET;
  }
  return null;
}
...
export function emptyCatalog() {
  return {
    ...
    playlists: [
      {
        id: 'pl-main',
        name: "Sonic Singularity Master Catalog · SS VIBELANDIA QUESTFEST",
        kind: 'sovereign',
...
/** Static manifest wins on id conflict for tracks; overlay playlists upsert by id (shared user lists). */
export function mergeCatalogSnapshots(base, overlay) {
  ...
  const tracks = { ...b.tracks, ...a.tracks };
  const byId = new Map(a.playlists.map((p) => [p.id, { ...p, trackIds: [...p.trackIds] }]));

  for (const p of b.playlists) {
    if (p.id === 'pl-main') continue;
    // Static manifest owns pinned sovereign playlists — dynamic overlay must not wipe track lists.
    if (PINNED_SOVEREIGN_PLAYLIST_IDS.has(p.id)) continue;
    // Upsert — updates must replace stale copies, not only add missing ids.
    byId.set(p.id, { ...p, trackIds: [...p.trackIds] });
  }

  let master = byId.get('pl-main') || a.playlists[0] || b?.playlists?.[0] || null;
  if (!master) {
    const pl = emptyCatalog().playlists[0];
    master = { ...pl };
    byId.set(pl.id, master);
  }
...
/** Raw manifest — no upload-index reconcile (safe for writes/deletes). */
async function loadDynamicCatalogBase() {
  let redisCat = null;
  ...
  if (redisCat && blobCat) {
    ...
    if (bv > rv) return blobCat;
    if (rv > bv) return redisCat;
    return blobCat;
  }
  return redisCat || blobCat || null;
}

/** Dynamic overlay only (no static merge) — for playlist/track writes. */
export async function loadDynamicCatalogOverlay() {
  return loadDynamicCatalogBase();
}

/** Prefer newest overlay when both Upstash and Blob have manifests. */
export async function loadDynamicCatalog() {
  return mergeTrackIndexOverlay(await loadDynamicCatalogBase());
}

function titleFromBlobPathname(pathname) {
  const base = String(pathname || '')
    .replace(/^catalog\/trk-srv-[0-9a-f-]+-/i, '')
    .replace(/\.[^.]+$/, '');
  return base.replace(/[_-]+/g, ' ').trim() || 'Untitled';
}

/** Ensure pl-main lists every track in the manifest (master catalog headcount). */
export function reconcileMasterTrackIds(catalog) {
  const base = normalizeCatalog(catalog);
  const playlists = base.playlists.map((p) => {
    if (p.id !== 'pl-main') return p;
    const ids = [...p.trackIds];
    const seen = new Set(ids);
    for (const id of Object.keys(base.tracks)) {
      if (!seen.has(id)) {
        ids.push(id);
        seen.add(id);
      }
    }
    if (ids.length === p.trackIds.length) return p;
...
async function mergeTrackIndexOverlay(catalog, { persist = true } = {}) {
  if (!upstashConfigured()) return reconcileMasterTrackIds(catalog || emptyCatalog());
  ...
  try {
    ...
    return reconcileMasterTrackIds(catalog || emptyCatalog());
  }
  if (!entries.length) return reconcileMasterTrackIds(catalog || emptyCatalog());

  /** LPUSH stores newest first — keep newest row per id. */
  ...
  for (const raw of entries) {
    try {
      ...
      if (!id || !track?.src || indexed.has(id)) continue;
      indexed.set(id, { ...track, id, serverHosted: true });
    } catch {
      /* skip corrupt index row */
    }
  }

  let next = reconcileMasterTrackIds(catalog || emptyCatalog());
  let changed = false;
  for (const [id, track] of indexed) {
    if (next.tracks[id]) continue;
    if (await redisSismember(CATALOG_TRACK_DELETED_KEY, id)) continue;
    next = appendTrackToDynamicCatalog(next, track);
    changed = true;
  }

  const masterBefore =
    next.playlists.find((p) => p.id === 'pl-main')?.trackIds.length ?? Object.keys(next.tracks).length;
  next = reconcileMasterTrackIds(next);
  const masterAfter =
    next.playlists.find((p) => p.id === 'pl-main')?.trackIds.length ?? Object.keys(next.tracks).length;
  if (masterAfter > masterBefore) changed = true;
...
/** Recover server-hosted audio blobs missing from the manifest (upload ok, register clobbered). */
export async function reconcileOrphanBlobTracks(catalog) {
  if (!process.env.BLOB_READ_WRITE_TOKEN) return { catalog: catalog || emptyCatalog(), added: 0 };
  ...
  do {
    const page = await list({ prefix: 'catalog/trk-srv-', cursor, limit: 1000 });
    for (const blob of page.blobs) {
      const pathname = blob.pathname || '';
      const match = pathname.match(BLOB_TRACK_PATH_RE);
      if (!match) continue;
      const id = match[1];
      const ext = (match[2] || '').toLowerCase();
      if (!BLOB_AUDIO_EXT.has(ext)) continue;
      if (next.tracks[id]) continue;
      if (upstashConfigured() && (await redisSismember(CATALOG_TRACK_DELETED_KEY, id))) continue;

      next = appendTrackToDynamicCatalog(next, {
        id,
        title: titleFromBlobPathname(pathname),
  ...
  next = reconcileMasterTrackIds(next);
  return { catalog: next, added };
}

/**
 * Full repair pass — Redis upload index + orphan blob scan.
 * @returns {{ catalog: object, before: number, after: number, indexRecovered: number, blobRecovered: number }}
 */
export async function reconcileCatalogFully({ includeBlobOrphans = true } = {}) {
  const base = (await loadDynamicCatalogBase()) || emptyCatalog();
  const before = Object.keys(base.tracks).length;

  const afterIndex = await mergeTrackIndexOverlay(base, { persist: true });
  const afterIndexCount = Object.keys(afterIndex.tracks).length;

  let catalog = afterIndex;
  let blobRecovered = 0;
  if (includeBlobOrphans) {
    const blobResult = await reconcileOrphanBlobTracks(catalog);
    catalog = blobResult.catalog;
    blobRecovered = blobResult.added;
    if (blobRecovered > 0) {
      await saveDynamicCatalog(catalog);
    }
  }

  const after = Object.keys(catalog.tracks).length;
  return {
    catalog,
    before,
    after,
    indexRecovered: Math.max(0, afterIndexCount - before),
    blobRecovered,
  };
}
...
export async function registerTrackPersistently(track, maxAttempts = 12) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    const dynamic = (await loadDynamicCatalogBase()) || emptyCatalog();
    ...
    const next = reconcileMasterTrackIds(appendTrackToDynamicCatalog(dynamic, track));
    const saved = await saveDynamicCatalog(next);
    if (!saved.ok) {
      return { ok: false, message: saved.message || 'catalog_save_failed' };
    }

    const verify = (await loadDynamicCatalogBase()) || emptyCatalog();
    if (verify?.tracks?.[id]) {
...
export async function removeTracksPersistently(trackIds, loadMergedCatalog, req, maxAttempts = 10) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    let dynamic = (await loadDynamicCatalogBase()) || emptyCatalog();
    ...
    await markTracksDeletedInIndex([...removed, ...missing]);

    for (const tr of blobTargets) {
      try {
        await deleteTrackMediaBlobs(tr, { catalog: next });
      } catch (e) {
...
export async function loadServerCatalog(req) {
  ...
  if (!dynamic) return reconcilePinnedSovereignPlaylists(staticCatalog);
  return reconcilePinnedSovereignPlaylists(mergeCatalogSnapshots(staticCatalog, dynamic));
}
...
/**
 * Collect blob pathnames still referenced by other catalog tracks.
 * Used so deleting a duplicate does not nuke a shared `src` still in use.
 * @param {object|null|undefined} catalog
 * @param {string} [exceptTrackId]
 */
export function blobPathnamesReferencedByCatalog(catalog, exceptTrackId) {
  const refs = new Set();
  const tracks = catalog?.tracks && typeof catalog.tracks === 'object' ? catalog.tracks : {};
  for (const [id, tr] of Object.entries(tracks)) {
    if (!tr || id === exceptTrackId) continue;
    for (const url of [tr.src, tr.videoSrc, tr.posterSrc]) {
...
export async function deleteTrackMediaBlobs(track, opts = {}) {
  ...
  const retained = blobPathnamesReferencedByCatalog(opts.catalog, track.id);
  const paths = [...pathSet].filter((p) => !retained.has(p));
  const skippedShared = pathSet.size - paths.length;
  if (skippedShared > 0) {
    console.warn(
      '[catalog] skip shared blob delete',
      track.id,
      'retained',
      skippedShared,
      'of',
  ...
  if (!paths.length) return { ok: true, deleted: 0, skippedShared };

  try {
    await del(paths);
    return { ok: true, deleted: paths.length, skippedShared };
  } catch (e) {
    console.error('[catalog] blob delete track media', track.id, e);
    return { ok: false, deleted: 0, reason: e?.message || 'blob_delete_failed', skippedShared };
  }
}
...
const RESERVED_PLAYLIST_IDS = new Set(['pl-main', 'pl-my-likes', 'pl-concierto-prelude', 'pl-reception', 'pl-sin-city', 'pl-reading-room']);

function sanitizeUserPlaylist(p, trackIdsInCatalog) {
  if (!p || typeof p !== 'object' || RESERVED_PLAYLIST_IDS.has(p.id)) return null;
  const id = String(p.id || '')
    .replace(/[^\w-]/g, '')
    .slice(0, 80);
  if (!id) return null;
  const trackIds = Array.isArray(p.trackIds)
    ? p.trackIds.filter((tid) => trackIdsInCatalog.has(String(tid)))
    : [];
  const name = clampText(p.name, 120) || 'Playlist';
  const description = p.description !== undefined ? clampText(p.description, 500) : undefined;
  const genre = p.genre !== undefined ? clampText(p.genre, TRACK_GENRE_MAX) : undefined;
  const next = {
    id,
    name,
    kind: p.kind === 'sovereign' ? 'sovereign' : 'user',
    trackIds,
    ...(description ? { description } : {}),
    ...(genre ? { genre } : {}),
    ...(p.posterSrc ? { posterSrc: String(p.posterSrc).trim().slice(0, 2048) } : {}),
    ...(Array.isArray(p.childPlaylistIds) && p.childPlaylistIds.length
      ? { childPlaylistIds: p.childPlaylistIds.map((c) => String(c).slice(0, 80)).filter(Boolean) }
      : {}),
  };
  return next;
}

/** Merge shared user playlists into the dynamic catalog (all listeners see them). */
export function upsertUserPlaylists(catalog, incomingPlaylists, opts = {}) {
  const base = catalog ? normalizeCatalog(catalog) : emptyCatalog();
  const allow =
    Array.isArray(opts.allowedTrackIds) && opts.allowedTrackIds.length
      ? new Set(opts.allowedTrackIds.map(String))
      : new Set(Object.keys(base.tracks));
  const byId = new Map(base.playlists.map((p) => [p.id, { ...p, trackIds: [...p.trackIds] }]));

  for (const raw of Array.isArray(incomingPlaylists) ? incomingPlaylists : []) {
    const p = sanitizeUserPlaylist(raw, allow);
    if (!p) continue;
    byId.set(p.id, p);
  }
...
/** Remove playlists by id from dynamic catalog (explicit delete from a client). */
export function removeUserPlaylists(catalog, deleteIds) {
  const base = catalog ? normalizeCatalog(catalog) : emptyCatalog();
  const drop = new Set(
    (Array.isArray(deleteIds) ? deleteIds : [])
      .map((id) => String(id || '').replace(/[^\w-]/g, '').slice(0, 80))
      .filter((id) => id && !RESERVED_PLAYLIST_IDS.has(id)),
  );
  if (!drop.size) return base;
  return {
    ...base,
    playlists: base.playlists.filter((p) => !drop.has(p.id)),
    version: Math.max(Number(base.version) || 1, 1) + 1,
  };
}
