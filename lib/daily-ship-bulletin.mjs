...
/** Calendar day (YYYY-MM-DD) from registry published — supports date-only or ISO timestamps. */
function publishedYmd(published) {
...
function pickFeaturedPapers(limit = 3, asOfYmd = null) {
   ...
   *  Compare by calendar day so ISO timestamps (…T18:00:00Z) are not treated as “future”
   *  relative to a date-only asOfYmd. */
  const rows = Object.entries(WHITEPAPER_REGISTRY)
    .filter(([, e]) => {
      if (e.featured !== true || e.surfaceVisible === false || !e.published) return false;
      if (asOfYmd) {
        const day = publishedYmd(e.published);
        if (!day || day > String(asOfYmd)) return false;
      }
      return true;
    })
    .map(([id, entry]) => ({
...
function whyCareLead(id) {
  if (id === 'synthobs-awareness-vs-brute-force-leverage-2026-09') {
    return (
      `Blind stacks still burn six figures to guess. ` +
      `Today’s tip is <a href="/ship-blog/awareness-vs-brute-force">awareness vs brute-force leverage</a> under Φ ≈ 1.618 — ` +
      `catalog contrast to AlphaFold-class training tax, not CASP retirement. Fair Exchange tip clause on.`
    );
  }
    if (id === 'synthobs-macro-protein-work-engine-2026-09') {
    return (
      `Living bodies still get filed as accidents of chemistry. ` +
      `Today’s tip is <a href="/ship-blog/macro-protein-work-engine">macro-protein work engines</a> under Φ ≈ 1.618 — ` +
      `θ_bio ∈ [13,17] with Kleiber/WBE framing, not wet-lab QED. Fair Exchange tip clause on.`
    );
  }
  if (id === 'synthobs-prime-indexed-volumetric-storage-2026-09') {
    return (
      `Storage stacks still pay a parity tax and wait on lookup tables. ` +
      `Today’s tip is <a href="/ship-blog/prime-indexed-volumetric-storage">prime-indexed volumetric vaults</a> under Φ ≈ 1.618 — ` +
      `catalog framing vs Reed-Solomon / LDPC / LBA, not a JEDEC silicon claim. Fair Exchange tip clause on.`
    );
  }
  if (id === 'synthobs-protein-folding-prime-container-2026-09') {
    return (
      `Protein folding still gets sold as a black-box contest. ` +
      `Today’s tip is <a href="/ship-blog/protein-folding-prime-container">prime-container folding</a> under Φ ≈ 1.618 — ` +
      `catalog contrast to AlphaFold, not CASP gold or clinical advice. Fair Exchange tip clause on.`
    );
  }
  if (id === 'synthobs-moving-up-the-stack-valuation-2026-09') {
    return (
      `<a href="/frontier">Frontier</a> CEOs: start with Executive Papers. ` +
      `Today’s tip is <a href="/ship-blog/moving-up-the-stack">moving up the stack</a> — chip/LLM players buy upward; ` +
      `Lattice Chat is the next layer (cool token burn · harmonize agents · scale). New-layer framing $12B–$28B, not a peer-shelf misread. ` +
      `Paired with the EGS Lattice-Linear Gateway. Fair Exchange, not an audited appraisal.`
    );
  }
  if (id === 'synthobs-prime-vault-alphafold-race-2026-09') {
    return (
      `Race day — today’s tip is <a href="/ship-blog/prime-vault-alphafold-race">Prime-Vault vs ColabFold</a>: ` +
      `three FastA tiers (Ubiquitin · IL-2 complex · orphan/synthetic) with ColabFold as the AlphaFold-class lane (` +
      `<code>COLABFOLD_LIVE=1</code>). Honesty: no invented pLDDT in CI. Fair Exchange tip clause on.`
    );
  }
  if (id === 'synthobs-sing-muse-omniversal-lattice-2026-09') {
    return (
      `Sing, O Muse — today’s tip is the voyage epic <a href="/ship-blog/sing-muse-omniversal-lattice">Sing, Muse, of the Omniversal Lattice</a>: ` +
      `Goldilocks SuperAI as play and set, primes as players, composites as NPCs, Φ ≈ 1.618 as the Story’s golden key, ` +
      `convergence at the Truckee River. Mythos grammar, not physics QED. Fair Exchange tip clause on.`
    );
  }
  if (id === 'synthobs-what-it-means-to-be-frontier-2026-09') {
    return (
      `<a href="/frontier">Frontier</a> CEOs: start with Executive Papers. ` +
      `Today’s tip pairs the EGS Lattice-Linear Gateway (PDVSA ops mock) with <a href="/ship-blog/moving-up-the-stack">moving up the stack</a> ` +
      `(valuation framing). Then play the <a href="/frontier">Frontier trail</a> — self-test + Where Am I on the QUESTFEST board. ` +
      `Hospitality grammar and simulator demos, not live oilfield SLAs or a caste test.`
    );
  }
  if (id === 'synthobs-infinite-octave-prime-parity-2026-09') {
    return (
      `Sole-even prime 2 as binary dyad anchor; odd primes as irreducible minimum sets under Φ ≈ 1.618. ` +
      `Catalog math + solar filing labels, not QED or NOAA proof.`
    );
  }
  if (id === 'synthobs-pdvsa-gateway-ops-mockup-2026-09') {
    return (
      `<a href="/frontier">Frontier</a> CEOs: start with Executive Papers. ` +
      `Today’s tip is the EGS Lattice-Linear Gateway (PDVSA ops mock) plus <a href="/ship-blog/moving-up-the-stack">moving up the stack</a> ` +
      `(valuation framing). Nine clickable takeaways into the papers and math — simulator, not live oilfield SLAs.`
    );
  }
  if (id === 'synthobs-lattice-vs-vibe-coding-2026-09') {
    return (
      `<a href="/frontier">Frontier</a> CEOs: start with Executive Papers. ` +
      `Today’s tip is Lattice vs vibe-coding design/write/deploy receipts — ` +
      `paired with <a href="/ship-blog/moving-up-the-stack">moving up the stack</a> valuation framing and the EGS Lattice-Linear Gateway mock. Committed diffs, not slideware.`
    );
  }
  ...
  if (id === 'synthobs-infinite-octaves-omniversal-lattice-2026-08') {
    return (
      ...
      `Today’s news is Infinite Octave Omniversal Lattice Catalog — recursive nesting for the grand Story. ` +
...
export async function buildDailyShipBulletin(opts = {}) {
  ...
  const highlights = papers.map(({ id, entry }) => ({
    id,
    title: guestHeadline(id, entry),
    href: shipBlogHref(id),
    paperHref: whitepaperSurfaceHref(id),
    blurb: guestBlurbForPaper(id, entry),
  }));

  // Prefer live QUESTFEST headlines over stale bulletin-board title strings.
  const leadResolved = leadBoard
    ? {
        id: leadBoard.id,
        title: guestHeadline(leadBoard.docRef || '', { title: leadBoard.title }),
        href: leadBoard.href,
      }
    : null;

  const htmlBody = composeNewsBody(highlights, leadResolved);
