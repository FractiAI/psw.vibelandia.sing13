...
import {
  attachHashpowerToForecast,
  attachHashpowerToResolved,
  getSimulatedHashrateHs,
} from './goldilocks-hashpower.mjs';
...
/** Cadence leg: EGS φ interval band hit. */
export function isCadenceSuccessful(score) {
  return score?.intervalInPhiBand === true;
}

/** Full pipeline: φ cadence + simulated PoW lottery win. */
export function isPredictionSuccessful(score) {
  return score?.fullPipelineWin === true;
}

function buildWouldHaveWon(totalRewardSats, score) {
  if (!isPredictionSuccessful(score) || totalRewardSats <= 0) return null;
  const amt = formatWonAmount(totalRewardSats);
  return {
    ...amt,
    plainSpeak:
      'Hypothetical block reward (subsidy + fees) if our sim fleet won cadence (φ band) and the PoW lottery for this height — not paid to us; scoreboard only.',
  };
}

/** EGS cadence + hashpower forecast for next block (public chain + mining API). */
export async function computeNextBlockPrediction(tipBlock, recentBlocks, mempool) {
  ...
  const hp = await attachHashpowerToForecast(body, tipBlock, predictedIntervalSec);
  body.miningSnapshot = hp.miningSnapshot;
  body.simulatedHashrateHs = hp.simulatedHashrateHs;
  body.powSimulation = hp.powSimulation;
...
export async function reconcilePrediction(openPrediction, actualBlock) {
  ...
  const hp = await attachHashpowerToResolved(openPrediction, actualBlock);
  const powWin = hp.powSimulation?.simulatedPowWin === true;
  const fullPipelineWin = intervalInBand && powWin;
  ...
  const entry = {
    ...
    miningSnapshot: hp.miningSnapshot,
    simulatedHashrateHs: hp.simulatedHashrateHs,
    powSimulation: hp.powSimulation,
    score: {
      intervalErrorSec,
      timestampErrorSec,
      feeErrorSats,
      ...
      cadenceWin: intervalInBand,
      simulatedPowWin: powWin,
      fullPipelineWin,
      subsidyExact: actualSubsidySats === f.predictedSubsidySats,
      predictionSuccessful: fullPipelineWin,
    },
    wouldHaveWon: buildWouldHaveWon(actualTotalRewardSats, {
      fullPipelineWin,
    }),
  };
...
function summaryStats(resolved) {
  ...
  const powWins = resolved.filter((r) => r.score?.simulatedPowWin).length;
  const fullWins = resolved.filter((r) => r.score?.fullPipelineWin);
  const totalWouldHaveWonSats = fullWins.reduce((s, r) => s + (r.wouldHaveWon?.sats ?? 0), 0);
  const avgIntervalErr =
    Math.round(
      (resolved.reduce((s, r) => s + Math.abs(r.score?.intervalErrorSec ?? 0), 0) / n) * 10,
    ) / 10;
  const lastPow = resolved[0]?.powSimulation;
  return {
    resolvedCount: n,
    cadenceHits: inBand,
    powSimWins: powWins,
    successfulCount: fullWins.length,
    intervalInPhiBandPct: Math.round((inBand / n) * 1000) / 10,
    powSimWinPct: Math.round((powWins / n) * 1000) / 10,
    fullPipelineWinPct: Math.round((fullWins.length / n) * 1000) / 10,
    avgAbsIntervalErrorSec: avgIntervalErr,
    simulatedHashrateLabel: lastPow?.simulatedHashrateLabel ?? null,
    networkHashrateLabel: lastPow?.networkHashrateLabel ?? null,
    totalWouldHaveWon:
      fullWins.length > 0 ? formatWonAmount(totalWouldHaveWonSats) : null,
  };
}
...
export async function runPredictionCycle(tipBlock) {
  ...
  if (state.open && tipBlock.height >= state.open.targetHeight) {
    ...
    if (actual) {
      resolvedEntry = await reconcilePrediction(state.open, actual);
      if (resolvedEntry) {
        resolved = [
          resolvedEntry,
  ...
  if (needNewOpen) {
    open = await computeNextBlockPrediction(tipBlock, recent, mempool);
  }
...
export const AWARD_VS_PREDICTION_NOTE = {
  plainSpeak:
    'Full simulation = (1) EGS φ cadence forecast, (2) simulated hashpower vs live network from mempool.space, ' +
    '(3) deterministic PoW lottery when the block is awarded. Hypothetical payout only if cadence AND sim PoW win.',
  wePredict: [
    'target block height (tip + 1)',
    'interval seconds + phi band',
    'sim hashrate vs network hashrate + difficulty',
    'hashes in window vs expected hashes per block',
    'subsidy + fee estimate',
  ],
  mainnetAwards: [
    'valid block hash below difficulty (real PoW)',
    'coinbase to miner payout address',
    'same block height we score',
  ],
  simulatedNotLive: [
    'no ASICs on this server',
    'PoW win roll is reproducible from block hash + configured sim hashrate',
    'HMAC on JSON is a receipt key, not a mining key',
  ],
  simHashrateConfig: {
    envFixed: 'COHERENCE_SIM_HASHRATE_H (default 1.618 PH/s)',
    envFraction: 'COHERENCE_SIM_HASHRATE_FRACTION (optional share of network)',
    currentSimHs: getSimulatedHashrateHs(1e21),
  },
};
...
export async function getPredictionLedger() {
  ...
  return {
    ...
    awardVsPrediction: AWARD_VS_PREDICTION_NOTE,
    open: state.open,
...
export async function backfillPredictionLedger({ count = 24 } = {}) {
  ...
  for (let i = 1; i < ascending.length; i++) {
    ...
    const syntheticOpen = await computeNextBlockPrediction(prev, windowNewestFirst, null);
    syntheticOpen.backfill = true;
    syntheticOpen.issuedAt = new Date(prev.timestamp * 1000 + 1000).toISOString();

    const entry = await reconcilePrediction(syntheticOpen, actual);
    if (!entry) continue;
    entry.backfill = true;
  ...
  const recent = await fetchRecentBlocks(10);
  const mempool = await fetchMempoolSnapshot();
  const open = await computeNextBlockPrediction(tip, recent, mempool);
