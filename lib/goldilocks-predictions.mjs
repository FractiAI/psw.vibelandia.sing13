...
export const AWARD_VS_PREDICTION_NOTE = {
  plainSpeak:
    'We predict block HEIGHT timing (seconds + φ band) and reward size for the next mainnet block. ' +
    'Bitcoin awards blocks via Proof-of-Work: miners find a valid hash; coinbase pays their address. ' +
    'Our HMAC signatures are receipt keys only — not mining keys, not coinbase scripts.',
  wePredict: ['target block height (tip + 1)', 'interval seconds until that block', 'phi timing band', 'subsidy + fee estimate'],
  mainnetAwards: ['valid block hash below difficulty target', 'coinbase to miner payout address', 'same block height we score'],
  notTheSameAs: ['mining private keys', 'pool stratum keys', 'block template selection keys'],
};
