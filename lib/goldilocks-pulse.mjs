...
export const HYDROGEN_LINE_MHZ = 1420.4;
...
export async function runGoldilocksPulse({ force } = {}) {
  ...
  if (!force && !shouldEmitPulse(state.latest, block)) {
    ...
    if (block.height > (state.latest?.bitcoin?.tipHeight ?? 0)) {
      predictions = await runPredictionCycle(block);
    } else {
      const { getPredictionLedger } = await import('./goldilocks-predictions.mjs');
      predictions = await getPredictionLedger();
    }
