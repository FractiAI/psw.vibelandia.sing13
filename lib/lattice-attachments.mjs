/**
 * Lattice Chat guest attachments — images + text docs + PDF text extract on the BYOK pipe.
 ...
 * remains text-first until that agent pipe takes inline media. PDFs arrive as
 * edge-extracted text (not native PDF vision); scanned/image-only PDFs may fail.
 */
...
/**
 * @param {{ unlimited?: boolean } | undefined} opts
 */
function attachCaps(opts = {}) {
  const unlimited = Boolean(opts?.unlimited);
  return {
    unlimited,
    maxFiles: unlimited ? Number.POSITIVE_INFINITY : LATTICE_ATTACH_MAX_FILES,
    maxChars: unlimited ? Number.POSITIVE_INFINITY : LATTICE_ATTACH_MAX_TOTAL_CHARS,
  };
}
...
/**
 * @param {unknown} raw
 * @param {{ unlimited?: boolean }} [opts] — Player 1 / creator: no Lattice file/char caps
 * @returns {{ name: string, mime: string, kind: 'image' | 'doc', text?: string, dataBase64?: string }[]}
 */
export function normalizeLatticeAttachments(raw, opts = {}) {
  if (!Array.isArray(raw)) return [];
  const { maxFiles, maxChars } = attachCaps(opts);
  const out = [];
  const items = Number.isFinite(maxFiles) ? raw.slice(0, maxFiles) : raw.slice();
  for (const item of items) {
    ...
    if (typeof item.text === 'string' && item.text.trim()) {
      row.text = Number.isFinite(maxChars)
...
export function foldAttachmentsIntoMessage(message, attachments, opts = {}) {
  ...
  if (!unlimited && folded.length > maxChars) {
    folded = `${folded.slice(0, maxChars)}\n\n[…attachments truncated for Goldilocks size…]`;
  }
  return folded;
}
