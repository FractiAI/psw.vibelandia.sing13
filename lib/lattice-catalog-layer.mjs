 ...
 *
 * Guest copy is CEO plain speak: where the layer sits, what people feel,
 * what each innovation delivers, and what is impossible without it.
 */
...
export const LATTICE_CATALOG_LAYER = Object.freeze({
  ...
  lead:
    'The mediation layer between your models and your data — easier briefs for teams, clearer answers for customers, and a calmer bill for leadership. No sixth cruise door. No new math for the people who only need better outcomes.',
  stackLine:
    'Models &amp; IDEs → <strong>Infinite Octaves catalog layer</strong> → ship doors &amp; enterprise data',
...
export const LATTICE_CATALOG_LAYER_CONTENT = Object.freeze({
  stackLinePlain:
    'Models & agentic IDEs → Infinite Octaves catalog layer → ship doors & enterprise data',
  oneLiner:
    'Infinite Octaves Omniversal Lattice Chat is the catalog layer in the AI stack — it sits between models and data so teams ship clearer work and customers only see better answers, without learning the technology underneath.',
  transparencyLine:
  ...
  accessLine:
    'Open the briefing sandbox → run a with/without scenario → deploy as a mediation layer (handshake · inject · calibrate).',
  honestyShort:
    'Catalog architecture for agentic coordination — not a finished physics proof, and not a promise that GPUs disappear.',
});
...
export function renderLatticeCatalogLayerBandHtml({ compact = false } = {}) {
  ...
  const lead = compact
    ? 'Catalog layer between models and data — clearer briefs for teams, better answers for customers.'
    : L.lead;
