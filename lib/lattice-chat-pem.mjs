...
const LEARN_SHELF_BEGIN = '<!-- AUTO:OMNI-LATTICE-LEARNING-SHELF:BEGIN -->';
const LEARN_SHELF_END = '<!-- AUTO:OMNI-LATTICE-LEARNING-SHELF:END -->';

export const OMNI_LATTICE_COURSE_FILE = 'interfaces/omni-lattice-course.html';
export const OMNI_LATTICE_TEXTBOOK_FILE = 'interfaces/omni-lattice-textbook.html';
...
/**
 * Rewrite AUTO blocks in the PEM + AGENT_SYNC + course/textbook living shelves
 * from ENGINE_SHELF.
 */
export async function syncLatticeChatPem({ cwd = process.cwd(), now = new Date() } = {}) {
  const shelf = listEngineShelf(true);
  const iso = now.toISOString().slice(0, 10);
  const learningHtml = renderLearningShelfHtml(shelf, now);
  ...
  const courseAbs = join(cwd, OMNI_LATTICE_COURSE_FILE);
  let course = await readFile(courseAbs, 'utf8');
  course = replaceMarkedBlock(
    course,
    LEARN_SHELF_BEGIN,
    LEARN_SHELF_END,
    learningHtml,
    OMNI_LATTICE_COURSE_FILE,
  );
  await writeFile(courseAbs, course, 'utf8');

  const textbookAbs = join(cwd, OMNI_LATTICE_TEXTBOOK_FILE);
  let textbook = await readFile(textbookAbs, 'utf8');
  textbook = replaceMarkedBlock(
    textbook,
    LEARN_SHELF_BEGIN,
    LEARN_SHELF_END,
    learningHtml,
    OMNI_LATTICE_TEXTBOOK_FILE,
  );
  await writeFile(textbookAbs, textbook, 'utf8');

  return {
    ok: true,
    file: LATTICE_CHAT_PEM_FILE,
    agentSync: AGENT_SYNC_FILE,
    course: OMNI_LATTICE_COURSE_FILE,
    textbook: OMNI_LATTICE_TEXTBOOK_FILE,
