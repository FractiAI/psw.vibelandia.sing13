 ...
 * @param {{ id: string, name: string, hue?: string }[]} [peers]
 */
export function rewriteSharedDmForSeat(raw, myPeerId, peers = DEFAULT_PEERS) {
  if (!raw || typeof raw !== 'object' || !myPeerId) return null;
  ...
  const from = peers.find((p) => p.id === fromPeerId);

  return {
    id,
    type: 'chat',
    platform: 'lattice',
    actor: isMine ? 'You' : from?.name || 'Peer',
    body: text,
    threadPeerId,
    presenceHue: isMine ? 'gold' : from?.hue || 'purple',
    createdAt:
...
const DEFAULT_PEERS = [
  { id: 'peer_valet_pru', name: 'Valet Pru', hue: 'gold' },
  { id: 'peer_daniel', name: 'Daniel', hue: 'purple' },
];
