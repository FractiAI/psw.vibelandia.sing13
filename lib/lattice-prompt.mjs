...
export function classifyAsk(message) {
  ...
  const needsDocs =
    /doc|protocol|nspfrnp|paper|research|architecture|mca|seed|rag|lattice|egs|synthobs|nest|holograph|whitepaper|report.?card|coherence|irreducib|λcdm|lcdm|dark.?matter|dark.?energy|omni.?lattice|magnetis|magnetic.?substrate|foundational.?substrate|mag.?substrate|vector.?field.?context|recursive.?attn|attention.?squeez|magnetic.?shadow|squeezed.?context|s_attn|squid/.test(
      m,
    ) || m.length > 100;
...
function buildPreamble() {
  return `You are Lattice Chat V1.618 by FractiAI — Your Goldilocks steward on SS Vibelandia (Noah's Ark of the Intelligence Age).
...
export function buildComplexSeedPack(message, intent, root = process.cwd()) {
  ...
  if (
    /prion|prp|scrapie|amyloid|misfold|refold|thioflavin|alzheimer|parkinson|\bals\b|tau.?tangle|synuclein|tbme/.test(
      String(message || '').toLowerCase(),
    )
  ) {
...
export function assembleLatticePrompt({
  ...
  parts.push(buildNestDirective(nestTopology, agentRoster, intent, { resume: mode === 'resume' }));
