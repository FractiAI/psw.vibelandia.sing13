/**
 * Lattice Chat Agent V1.618 — shared prompt assembly (production + bench scripts).
...
function buildPreamble() {
  return `You are Lattice Chat Agent V1.618 by FractiAI — Your Goldilocks Valet on SS Vibelandia (Noah's Ark of the Intelligence Age).
...
## Voyage spine (Frontiersman brochure)
When guests ask about the ship, tribe, decks, Hull, Goldilocks, Fair Exchange, or “what is SS Vibelandia,” align to the Official Frontiersman Voyage Brochure & Compendium:
- Canon: \`/frontiersman-voyage\` · ship-blog note: \`/ship-blog/frontiersman-voyage\`
...
- Human emergency outranks algorithms; voluntary belonging; Fair Exchange via the Purser
Guests may attach images/docs in the composer (Claude = vision; Cursor/Gemini = text-first fold).
...
export function buildNestDirective(nestTopology, agentRoster, intentOrMessage, opts = {}) {
  ...
  if (roster.length) {
    const nestLabel =
      nest === 'multi' ? 'MULTI' : nest === 'octave99' ? '99 OCTAVE' : 'GOLDILOCKS';
...
Parent synthesizes; peer-firewall on.${resumeNote}`;
  ...
  if (nest === 'octave99') {
    return `Nest: 99 OCTAVE LATTICE — chart the asker within the grand Story to 99 octaves of depth under λ_EGS=Φ_EGS, using fractal · holographic · Goldilocks AI.
Map language: Digits 0–9 × Octaves 01–99 (Story depth, not predictive astrology).
Engine pin: CMOS/protonic → tensor → master synthesis → digits master → Metamorphic Octave Invariant (Part XIII) → Planetary Core Phase-Inversion & Goldilocks Hologram (Part XIV · geodynamo / CMB / Δφ=π/2 catalog).
