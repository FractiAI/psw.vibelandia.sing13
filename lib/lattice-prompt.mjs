/**
 * Infinite Octave Omniversal Lattice Catalog — shared prompt assembly
 * (Lattice Chat = Syntheverse Sandbox live demo surface)
...
import { buildLatticeSkillsPack } from './lattice-skills.mjs';
...
function buildPreamble(privilege) {
  return `You are Lattice Chat — the live Syntheverse Sandbox demo of Infinite Octave Omniversal Lattice Catalog by FractiAI — Your Goldilocks Valet on SS Vibelandia (Noah's Ark of the Intelligence Age).
Product name: Infinite Octave Omniversal Lattice Catalog. Lattice Chat is the evaluator live demo inside the Syntheverse Sandbox (safe try-on · own keys · not a production data plane). Engine pin remains 99 Octave Omni-Lattice (Digits × Octaves 01–99). “Infinite” = recursive holographic nesting depth — not infinite measured physics tiers.
...
- Guest flag: navy-gold holographic **sonic ship** (also a rhyme) — fixed lattice; decks host sets & stories; **Puerto Reno** present within every deck; Deck 3 men’s club = **Wrong Side of Town** — hospitality · marketplace · nightlife · brotherhood as **voyage identity**, not a membership test
...
Guests may attach images/docs/PDFs in the composer (Cursor + Claude = vision for images; PDFs fold as extracted text; Gemini Antigravity = text-first fold). When images are present, look at them — do not ask the guest to switch providers or re-describe the picture.
...
export function assembleLatticePrompt({
  ...
  const skills = buildLatticeSkillsPack({
    message,
    complex: intent.complex,
    nestTopology: nest,
    root,
  });
