 ...
 * not a claim that Skillarum is embedded.
 */
...
export const LATTICE_SKILLS_PILOT = Object.freeze({
  ...
  pinchChars: 1400,
  honesty:
    'FractiSkills full local corpus — SKILL.md packages generated from repo registries (papers, ship-blog, protocols, research, major interfaces) plus curated Lattice/Core meta skills. Pointer-first load only. Not a live public crawl, not a finished physics proof.',
});

/** @deprecated alias — prefer LATTICE_SKILLS_PILOT / corpus constants */
export const LATTICE_SKILLS_CORPUS = LATTICE_SKILLS_PILOT;
...
export function matchSkills(message, index) {
  ...
  const scored = skills.map((s) => {
    ...
    // Prefer exact id / long slug hits in large corpora
    const id = String(s.id).toLowerCase();
    if (id.length > 8 && text.includes(id)) score += 8;
    for (const route of s.routes || []) {
      const slug = String(route).toLowerCase().split('/').filter(Boolean).pop();
      if (slug && slug.length > 5 && text.includes(slug)) score += 5;
    }
    if (/skill|fractiskill|portable.?skill|skill\.md|skills\/index/.test(text) && /skill|corpus|catalog/.test(hay)) {
      score += 2;
    }
...
export function buildLatticeSkillsPack({
  ...
  const lines = [
    `## FractiSkills corpus (pointer-first)`,
