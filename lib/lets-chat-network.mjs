...
import { normalizeEmail, isValidEmailShape, listCreatorEmails } from './lattice-access.mjs';
 ...
 * @returns {Promise<{ ok: true, email: string, peerId: string, name: string, privilege: string } | { ok: false, message: string }>}
 */
export async function boardLetsChat(rawEmail) {
  ...
  await saveDoc(doc);
  return { ok: true, email, peerId, name, privilege };
}
...
export async function inviteByPeerId(fromPeerId, toPeerIdRaw) {
  ...
  if (!target) {
    return {
      ok: false,
      code: 'unknown_peer',
      message: 'That Let\'s Chat id is not aboard yet. Ask them to Come aboard first, then invite.',
    };
  }
  const ek = edgeKey(fromPeerId, toPeerId);
  if (doc.edges[ek]?.status === 'accepted') {
    return { ok: true, alreadyConnected: true, peer: { id: target.peerId, name: target.name, privilege: target.privilege } };
  }
  const reverseKey = inviteKey(fromPeerId, toPeerId);
  if (doc.invites[reverseKey]) {
    // They already invited you — accept automatically.
    delete doc.invites[reverseKey];
    doc.edges[ek] = { a: [fromPeerId, toPeerId].sort()[0], b: [fromPeerId, toPeerId].sort()[1], status: 'accepted', at: Date.now() };
    await saveDoc(doc);
    return {
      ok: true,
      accepted: true,
  ...
  const ik = inviteKey(toPeerId, fromPeerId);
  doc.invites[ik] = { toPeerId, fromPeerId, at: Date.now() };
  await saveDoc(doc);
  return {
    ok: true,
    pending: true,
