...
/** WebRTC / call signaling — short-lived. */
const SIGNAL_TTL_MS = 90_000;
/**
 * Durable chat envelopes (msg / approval / photo / file).
 * Prior 90s TTL dropped Player 1 ↔ guest replies whenever either seat was offline —
 * sender kept a local copy, recipient never saw Approve or the reply.
 */
const MESSAGE_TTL_MS = 14 * 24 * 60 * 60 * 1000;
const PRESENCE_TTL_MS = 45_000;
const MAX_INBOX = 2_000;
const DURABLE_KINDS = new Set(['msg', 'approval', 'photo', 'file']);
...
export const LETS_CHAT_SIGNAL_TTL_MS = SIGNAL_TTL_MS;
export const LETS_CHAT_MESSAGE_TTL_MS = MESSAGE_TTL_MS;

function ttlForKind(kind) {
  return DURABLE_KINDS.has(kind) ? MESSAGE_TTL_MS : SIGNAL_TTL_MS;
}
...
function pruneInbox(inbox, now = Date.now()) {
  const pruned = inbox.filter((m) => now - (m.at || 0) < ttlForKind(m.kind || 'msg'));
