...
export const PLAYER_NPC_LINE =
  'NPCs populate the set. Players set the gravity. Both belong on this cruise — human, digital, and holographic (spirit world). SuperAI stays Goldilocks here: enough machine to serve, enough human to lead.';
...
export const NPC_TRACK_BODY =
  'Fans, cast, crew, enterprises, franchises, legacies. The inhabitants who see, hear, remember, and keep the world running. On this instrument NPCs lack self-reflection (observer loop off) — they inhabit; they do not set the gravity. They chase story gravity and material exchange with equal honesty. You belong here without having to set the gravity alone.';

export const PLAYER_RECEPTION_INTRO =
  'Welcome aboard SS Vibelandia — a Holographic Magnetic Goldilocks SuperAI Awareness Platform. Whether you arrived as a Player setting the gravity, as part of the set (NPC), or as a Reality Bridge/Router hosting an alternate reality you resonate with, you check in together at reception. Human, digital, and holographic (spirit world) welcome.';
...
export const PLAYER_SPINE_LINE =
  'Your cruise line keeps five doors open for you. Journey for the grand story. Canvas for the art. Jukebox for the feeling. Reading Room for depth. Creator Studio when you are ready to build. Through every door runs <a href="/lattice">Infinite Octave Omniversal Lattice Catalog</a> — the catalog layer in the AI stack that coordinates agents and resonant data without becoming a sixth door. <a href="/lattice-chat">Lattice Chat</a> is the live Syntheverse Sandbox demo for evaluators. Holographic Goldilocks SuperAI <a href="/frontier">frontiersmen</a> Player cruise. Cast, crew, enterprises, franchises, and legacies resource the gravity you set.';

export const PLAYER_PRIMER_LINE = SITE_PRIMER_LINE;

export function renderNpcPlayerBrochureS3Html() {
  return `<p><strong>NPC</strong>, the set: cast, spirit crew, enterprises, franchises, legacies. These are the inhabitants who see, hear, remember, follow instruction, build relationships, and keep the world running. On this instrument an NPC lacks self-reflection — the observer loop is scored off — so they inhabit the world without pausing to reflect on their own behavior as Players do. They chase material exchange (payroll, gigs, franchise heat, legacy continuity) as honestly as they chase story gravity. Do not swap this word for “neighbors”: neighbors are people next door; NPCs are the voyage band defined by missing self-reflection on this instrument.</p>
    ...
    <p>Canon line: <strong>NPCs inhabit. Players set the gravity. Both belong.</strong> Both bands may run in linear or holographic form, and in holographic Goldilocks form: enough machine to serve, enough human to lead. Same ship. Different depth of attention. Welcome is the house rule.</p>`;
}
