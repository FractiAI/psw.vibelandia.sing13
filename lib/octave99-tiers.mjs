...
export const OCTAVE99_TIERS = Object.freeze({
  ...
  agent: {
    ...
    blurb:
      'Full Infinite Octaves Omniversal Lattice Chat Agent + IDE (Cursor) and OpenRouter BYOK — fractal · holographic · Goldilocks AI across Story depth (engine map Digits × Octaves 01–99). Bridge routes through SING13.',
  },
  chart_standard: {
    id: 'chart_standard',
    label: 'Your Infinite Octaves Chart',
    priceUsd: 29,
    cadence: 'reading',
  ...
  chart_deluxe: {
    id: 'chart_deluxe',
    label: 'Your Infinite Octaves Chart Deluxe',
    priceUsd: 49,
    cadence: 'reading',
...
export function honorRailLinks(amountUsd, opts = {}) {
  ...
  const memo = String(opts.memo || 'Your Infinite Octaves Chart · Fair Exchange').slice(0, 80);
...
export function honorAttestMailto(tierId, amountUsd) {
  ...
  const title = deluxe ? 'Your Infinite Octaves Chart · Deluxe $49' : 'Your Infinite Octaves Chart · Standard $29';
