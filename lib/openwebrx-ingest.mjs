...
export async function captureOpenWebRxFrame({
  ...
  timeoutMs = 22000,
  outputRate = 12000,
  preferSpectrum = false,
} = {}) {
  const list = Array.isArray(endpoints) ? endpoints : OPENWEBRX_ENDPOINTS;
  let lastErr = null;
  for (const ep of list) {
    try {
      return await captureOpenWebRxFromUrl(ep.url, ep.label, {
        tuneHz,
        timeoutMs,
        outputRate,
        preferSpectrum,
...
function captureOpenWebRxFromUrl(url, label, { tuneHz, timeoutMs, outputRate, preferSpectrum }) {
...
export async function getWebSdrFrame({
  live = false,
  maxAgeMs = WEBSDR_MAX_AGE_MS,
  endpoints,
  tuneHz,
  preferSpectrum = false,
} = {}) {
  ...
  if (live || stale) {
    try {
      const fresh = await captureOpenWebRxFrame({
        endpoints: endpoints ?? OPENWEBRX_ENDPOINTS,
        tuneHz: tuneHz ?? HYDROGEN_TUNE_HZ,
        preferSpectrum,
      });
