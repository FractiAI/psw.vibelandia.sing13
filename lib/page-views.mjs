...
export function normalizePageKey(raw) {
  ...
  // Ship board aliases (QUESTFEST / SS Vibelandia deck — not site root)
  if (path === '/interfaces/vibelandia-questfest') {
    path = '/questfest';
  }

  // Same jukebox surface at /listen and questfest-bridge
  if (path === '/interfaces/questfest-bridge' || path === '/questfest-bridge') {
    path = '/listen';
  }

  // Ark about page aliases share one counter
