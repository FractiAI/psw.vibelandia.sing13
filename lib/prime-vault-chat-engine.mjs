...
export const KNOWLEDGE_CELLS = Object.freeze({
  ...
  'SS Vibelandia': {
    ...
    speak:
      'SS Vibelandia is the navy-gold holographic sonic ship — also a rhyme: a fixed lattice where decks host the sets and stories. Puerto Reno is present within every deck. Deck 3 men’s club is Wrong Side of Town. One tribe, many homes. Guests meet voyage identity — hospitality · marketplace · nightlife · brotherhood — not a membership exam. The Official Prospectus carries Genesis → Borikén → Reno.',
  },
  'Stay Goldilocks': {
    primeIndex: 21,
  ...
  'Connection': {
    primeIndex: 26,
    tags: ['connect', 'rapport', 'listen', 'understand', 'conversation'],
    gist: 'Human connection check — affirm presence in the guest frame without dumping LUDCR / Race brochure text.',
    speak:
      "Yes — I hear you, and I'm with you. Say what you mean in your own words; I will answer from connection first, not a canned pipeline lecture.",
  },
});
...
const INTENT_HINTS = Object.freeze([
  { re: /\b(hello|hi|hey|greetings|hola|good (morning|afternoon|evening)|howdy|yo)\b/i, kind: 'greeting', cells: ['Voyage framing', 'Connection'] },
  {
  ...
  {
    // Rapport / presence — must win BEFORE howworks or corpus (Player 1: "Do you understand me?")
    re: /\b((do|did|can|could) you understand( me| us| this)?\b|are you (listening|with me|there|hearing me)|can you hear me|do you (hear|get) me|are we connecting|you (don'?t|arent|aren'?t) (listening|understanding|connecting)|not connecting|connect(ing)? through language|talk (with|to) me)\b/i,
    kind: 'connect',
    cells: ['Connection', 'Acknowledgment'],
  },
  {
  ...
  },
  {
    ...
    kind: 'clarify',
    cells: ['Connection', 'Acknowledgment', 'Domain boundary'],
  },
...
const TOKEN_SYNONYMS = Object.freeze({
  ...
  // Do NOT map understand/listen/chat → vault/retrieve — that poisons "Do you understand me?" into corpus dumps
  listen: ['listen', 'hear', 'present'],
  understand: ['understand', 'hear', 'connect'],
  comprehend: ['understand', 'hear', 'connect'],
  conversation: ['chat', 'listen', 'thread'],
  chat: ['chat', 'listen', 'thread'],
...
function detectKinds(text, history, vocabulary, cellBank) {
  ...
  if (history.length >= 2 && !kinds.has('greeting') && !kinds.has('ood') && !kinds.has('thanks') && !kinds.has('connect')) {
    kinds.add('continuing');
...
function speechAct(kinds, text, history, bestAnyLex = 0, bestCorpusLex = 0) {
  ...
  if (kinds.has('connect')) return 'connect';
  ...
  // Open metaphor / image language with content tokens but no vault hit → connect-invite, not cold menu
  if (bestAnyLex < 1.5 && kinds.has('explain') && /[?]/.test(text) && queryTokens(text).length >= 3) {
    return 'invite';
  }
...
/** Distinctive content words for connection / metaphor echo (even when ask is long). */
function heardTokens(text, max = 6) {
  const skip = new Set([
    ...STOPWORDS,
    'investigate',
    'rhyme',
...
function articulateFold({
  ...
  if (act === 'refuse') {
    ...
    parts.push(cellSpeak('Connection', cellBank) || "Yes — I hear you, and I'm with you.");
    const understandingAsk = /\b(understand|listening|hear me|with me)\b/i.test(text);
    if (understandingAsk && echo) {
      parts.push(`You asked whether I understand — I do hear “${echo}.” I will stay in your frame, not dump a pipeline brochure.`);
    } else if (echo) {
      parts.push(`I caught that — “${echo}.” Stay with me in plain speech; I will not answer with a Race/LUDCR brochure.`);
    } else if (heard.length) {
      parts.push(`I caught: ${heard.join(' · ')}. Keep going in your words — I am listening for meaning, not for a Race scoreboard cue.`);
    } else {
      parts.push('Ask again in your own words, or name a filed topic when you want a vault fold (Φ, voyage, Goldilocks, vaults).');
    }
  } else if (act === 'thanks') {
    ...
    // invite — connect through the guest's words first; do NOT dump El Gran Sol / Race brochure
    if (heard.length >= 2) {
      parts.push(
        `I hear you — ${heard.slice(0, 5).join(', ')}. That image isn't filed as a vault atom, so I won't invent a prophecy fold.`,
      );
      parts.push(
        'Still with you: tell me what it means for you, or map it onto a filed door (Φ, voyage, Goldilocks, vaults, Race).',
      );
    } else {
      parts.push(
...
export function createPrimeVaultChatEngine(opts = {}) {
  ...
  function queryLattice(userInput, queryOpts = {}) {
    ...
    for (let i = 0; i < vocabulary.length; i++) {
      ...
      if (act === 'connect' && /^(Connection|Acknowledgment)$/i.test(concept)) resonance *= 2.0;
    ...
    if (act === 'connect') {
      const c = ranked.filter((n) => /^(Connection|Acknowledgment)$/i.test(n.concept));
      const rest = ranked.filter((n) => !c.includes(n));
      if (c.length) ranked = [...c, ...rest];
    }
    ...
    return {
      ...
      abilities: {
        ...
        understand: {
          ...
          mapsTo: 'retrieve',
          tier: 'catalog',
          note: 'Expanded synonym + intent catalog match — connect/rapport before brochure. Not open-world comprehension',
          confidence: Number(bestAnyLex.toFixed(3)),
        },
