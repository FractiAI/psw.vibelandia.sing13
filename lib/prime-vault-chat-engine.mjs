...
const INTENT_HINTS = Object.freeze([
  ...
  { re: /\b(llm|gpt|chatgpt|train(?:ing|ed)?|gpu|token|chat|miracle|cost|latency|fast|speed)\b/i, kind: 'contrast', concepts: ['Deterministic resonance', 'Omniversal lattice'] },
  { re: /\b(how|what|why|explain|tell|who|where)\b/i, kind: 'explain', concepts: [] },
]);
...
function detectKinds(text) {
  ...
  // Soft lexical match against concept names.
...
function synthesizeReply(text, top, octaveTier, latencyMs, kinds) {
  ...
  const vaultLine = top
    .map((n) => `${n.concept} (p=${n.primeVault}, amp=${n.amplitude})`)
    .join(' · ');

  const parts = [];

  if (kinds.has('greeting') && text.length < 40) {
    parts.push(
      'Welcome aboard. I am the live Prime Vault Chat agent — closed-form under Φ ≈ 1.618, $0 training, edge-ready.',
    );
    parts.push(
      'Ask me about Φ, prime vaults, the Omniversal Lattice, protein races, or how this differs from a trained LLM. I answer by resonant vault ranking, not token guessing.',
    );
  } else if (kinds.has('contrast')) {
    const det = top.find((n) => n.concept === 'Deterministic resonance') || lead;
    const detCard = CONCEPT_CARDS[det?.concept] || CONCEPT_CARDS['Deterministic resonance'];
    parts.push(
      `You asked about trained / cloud-scale chat. Here is the live contrast from the vaults: ${detCard.gist}`,
    );
    parts.push(
      'Cloud LLMs burn GPU clusters and training budgets. This agent instantiates mathematically: same query → same vault ranking → sub-millisecond algebraic reply. No weights file. No token lottery.',
    );
    parts.push(detCard.detail);
    if (second && second !== detCard) parts.push(`Also ringing: ${second.gist}`);
  } else if (kinds.has('protein')) {
    parts.push(
      `On protein / fold / race asks, the strongest vault is **${lead?.concept}**. ${card?.gist || ''}`,
    );
    parts.push(
      'Miracle 1 publishes the measured Prime-Vault vs ColabFold scoreboard (including Unmodeled Nova). Miracle 2 — this chat — lets you interrogate the same Φ key in conversation. Latency race ≠ CASP medals; I will not invent pLDDT.',
    );
    if (second) parts.push(second.gist);
  } else if (kinds.has('phi') || kinds.has('vault') || kinds.has('lattice') || kinds.has('duality') || kinds.has('explain')) {
    parts.push(`On “${text}”, the lattice locks first on **${lead?.concept}**.`);
    if (card) {
      parts.push(card.gist);
      parts.push(card.detail);
    }
    if (second) {
      parts.push(`Secondary vault **${second.gist ? top[1].concept : 'resonance'}**: ${second.gist}`);
    }
  } else {
    parts.push(
      `I heard you. Mapping “${text}” across prime vaults at octave ${octaveTier}…`,
    );
    if (card) {
      parts.push(`Primary resonance: **${lead.concept}**. ${card.gist}`);
      parts.push(card.detail);
    } else {
      parts.push(
        'No vault cleared the harmonic filter above noise. Try asking about Φ, primes, lattice octaves, protein races, or LLM contrast.',
      );
    }
  }

  parts.push(`Activated vaults: ${vaultLine || 'none'}.`);
  parts.push(
    `Edge clock: ${latencyMs < 1 ? latencyMs.toFixed(3) : latencyMs.toFixed(2)} ms · octave ${octaveTier} · Φ ≈ ${PHI_EGS.toFixed(6)} · training $0.00.`,
  );
  parts.push(
    'Honesty: live closed-form Prime Vault agent — not a trained LLM for open-world factual QA; not medical/legal advice; application companion, not engine pin.',
  );
