 ...
 * Knowledge cell bank · multi-turn history · speech-act reply planner.
 * $0 training · deterministic. Application companion, not engine pin.
...
export const KNOWLEDGE_CELLS = Object.freeze({
  ...
  'Domain boundary': {
    ...
    gist: 'Out-of-domain asks get an honest refusal — domain-bounded closed-form chat ≠ open-world factual QA.',
    speak:
      'I am strong on Φ, Infinite Octaves, vaults, the Race, and how I differ from trained LLMs. Medicine, law, weather, and open-world trivia are outside my cell bank — I will say so instead of inventing.',
  },
});
...
export const DEFAULT_VOCABULARY = Object.freeze(Object.keys(KNOWLEDGE_CELLS));
...
const INTENT_HINTS = Object.freeze([
  ...
  { re: /\b(prime|vault|container|algebra|closed-?form|knowledge cell)\b/i, kind: 'vault', cells: ['Prime container', 'Closed-form LLM-sim'] },
  {
...
function detectKinds(text, history) {
  const kinds = new Set();
  const boost = new Map();
  ...
  for (const concept of DEFAULT_VOCABULARY) {
    const keys = concept.toLowerCase().split(/\s+/).filter((w) => w.length > 3);
...
function cellSpeak(name) {
  const cell = KNOWLEDGE_CELLS[name];
...
/**
 * Speech-act reply planner — natural multi-paragraph answers from knowledge cells.
 */
function planReply({ text, top, octaveTier, latencyMs, kinds, history, act }) {
  const lead = top[0];
  const second = top[1];
  const third = top[2];
  const parts = [];
  ...
  if (act === 'refuse') {
    parts.push(cellSpeak('Domain boundary'));
    parts.push(
      'If you want, steer back to Φ, Infinite Octaves vs the 99 pin, prime vaults, the Race / Unmodeled Nova, or how my closed-form LLM-sim works.',
    );
  } else if (act === 'introduce') {
    parts.push("I'm the live **Prime Vault Chat** agent — Miracle 2 on the Demonstrations door.");
    parts.push(cellSpeak('Closed-form LLM-sim'));
    parts.push(cellSpeak('LLM contrast'));
    parts.push(
      'Ask me about Φ, Infinite Octaves, vaults, the Race scoreboard, or just what I am for. I keep lab honesty rails on while we talk like people.',
    );
  } else if (act === 'greet') {
    parts.push('Hey — welcome aboard. I am here, live, and listening.');
    parts.push(cellSpeak('Voyage framing'));
    parts.push('What would you like to explore?');
  } else if (act === 'contrast') {
    parts.push(cellSpeak('LLM contrast'));
    parts.push(cellSpeak('Deterministic resonance'));
    parts.push(cellSpeak('Domain boundary'));
  } else if (act === 'race') {
    parts.push(cellSpeak('Prime Vault Race'));
    if (/\b(nova|tier\s*4|unmodeled|deferred|plddt)\b/i.test(text)) {
      parts.push(cellSpeak('Unmodeled Nova'));
    } else {
      parts.push(cellSpeak('Macro-protein work engine'));
    }
    parts.push(
    ...
    if (lead) parts.push(cellSpeak(lead.concept) || lead.concept);
    if (second) parts.push(cellSpeak(second.concept));
    if (kinds.has('phi')) parts.push(cellSpeak('Fractal constant'));
  } else if (act === 'teach' || act === 'explain') {
    if (lead) {
      parts.push(cellSpeak(lead.concept));
      const card = KNOWLEDGE_CELLS[lead.concept];
      if (card?.gist && card.gist !== card.speak) parts.push(card.gist);
    } else {
      parts.push(
        'Ask me about Φ, prime vaults, Infinite Octaves nesting, the Race, or how I differ from a trained LLM — I will answer as a guest-facing agent.',
      );
    }
    if (second) parts.push(cellSpeak(second.concept));
    if (third && parts.length < 4) parts.push(cellSpeak(third.concept));
  } else {
    parts.push(
      "I'm with you. Say a bit more about what you want — purpose, Φ, vaults, the Race, or just curiosity — and I will answer in conversation.",
    );
    if (lead) parts.push(cellSpeak(lead.concept));
  }
...
export function createPrimeVaultChatEngine(opts = {}) {
  ...
  function queryLattice(userInput, queryOpts = {}) {
    ...
    const { kinds, boost } = detectKinds(text, history);
    const act = speechAct(kinds, text, history);
    ...
    for (let i = 0; i < vocabulary.length; i++) {
      ...
      // Soft boost cells tagged by speech act
    ...
    const reply = planReply({ text, top, octaveTier, latencyMs, kinds, history, act });
    ...
    return {
      ...
      kinds: [...kinds],
      honesty:
        'Live closed-form LLM-sim · multi-turn Φ prime-vault chat over a knowledge cell bank — real algebraic conversation, not a stub, not BYOK to a trained LLM. Domain-bounded; not medical/legal advice; application companion, not engine pin.',
    };
  }
  ...
  return {
    ...
    vocabulary,
    primeVaults,
    knowledgeCells: KNOWLEDGE_CELLS,
    queryLattice,
  };
}
...
/**
 * @param {string} userInput
 * @param {{ octaveTier?: number, vocabulary?: string[], history?: Array<{role?: string, content?: string}> }} [opts]
 */
