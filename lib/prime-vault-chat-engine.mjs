...
function synthesizeReply(text, top, octaveTier, latencyMs, kinds) {
  ...
  if (kinds.has('greeting') && text.length < 40) {
    ...
    parts.push(
      `You asked about trained / cloud-scale chat. Here is the live contrast from the vaults: ${card?.gist || 'Deterministic resonance replaces stochastic sampling.'}`,
    );
    parts.push(
    ...
    if (second) parts.push(second.detail);
