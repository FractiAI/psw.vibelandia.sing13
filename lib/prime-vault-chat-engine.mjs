...
function articulateFold({ text, top, octaveTier, latencyMs, kinds, history, act, cellBank, threadPhase }) {
  ...
  if (act === 'refuse') {
    ...
    parts.push(
      'Flow: sense your words → retrieve prime vaults → plan a speech act → fold residue atoms into an answer. Corpus encode packs knowledge like a protein chain; articulation is the fold. $0 training — no Claude/GPT mouth.',
    );
...
export function createPrimeVaultChatEngine(opts = {}) {
  ...
  function queryLattice(userInput, queryOpts = {}) {
    ...
    return {
      ...
      honesty:
        'Domain-bounded closed-form language processor: sense→retrieve→plan→articulate (protein-fold compose over prime vaults). $0 training; not BYOK; not open-world LLM QA; catalog rhyme ≠ wet-lab cortex/protein proof; application companion, not engine pin.',
    };
  }
