  ...
  const eligible = Object.entries(WHITEPAPER_REGISTRY)
    ...
    .map(([id, e]) => {
      ...
      return {
        ...
        productHref: note?.productHref || '',
        productLabel: note?.productLabel || 'Product',
      };
    })
    .sort(
      (a, b) =>
  ...
  const items = posts
    .map((p) => {
      ...
      const secondary = p.hasBlogPost
        ...
        : '';
      const product = p.productHref
        ? `<a class="qf-blog-paper" href="${escapeAttr(p.productHref)}">${escapeHtml(p.productLabel || 'Product')}</a>`
        : '';
...
<head>
  ...
  <title>Ship blog · All plain-language notes · SS Vibelandia</title>
  <meta name="description" content="Every SS Vibelandia ship-blog note in plain language — newest first, honesty rails, links to whitepapers." />
