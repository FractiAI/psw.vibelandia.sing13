...
/**
 * Prefer papers that already have a hand-authored ship-blog note.
 * Fill remaining slots from newest registry papers (they still get a paper link until a note exists).
...
export function listRecentPaperBlogPosts(limit = 6) {
  ...
  const withNotes = eligible.filter((p) => p.hasBlogPost);
  const withoutNotes = eligible.filter((p) => !p.hasBlogPost);
  // Prefer authored notes first (still newest-first within each bucket via prior sort),
  // then fill with newest papers that lack a note yet.
  return [...withNotes, ...withoutNotes].slice(0, Math.max(1, limit));
}
...
export function renderQuestfestBlogHtml(posts = listRecentPaperBlogPosts(6)) {
  ...
  return `  <section class="qf-blog" id="ship-blog" aria-labelledby="qf-blog-h2">
    ...
    <h2 id="qf-blog-h2">Six newest notes</h2>
    <p class="qf-blog-lead">
      Real plain-language posts about what just landed — not title cards for the library.
