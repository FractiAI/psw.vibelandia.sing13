...
async function writeAuditReceipt({
  ...
  // Auto-publish to Lattice Collaborate shared paper pipe when PRA passes.
  try {
    const status = conv?.status;
    if (status === 'pass' || status === 'soft_pass') {
      const { publishCollaboratePaperEvent } = await import('./lattice-collaborate-papers.mjs');
      await publishCollaboratePaperEvent(receipt.paperId, {
        cwd: root,
        auditStatus: status,
        force: true,
      });
    ...
    /* non-blocking — audit receipt still written */
  }
