...
export async function runTurnerBisonPipeline() {
  ...
  const { fence, satellite, lstPass, steelPack, magnetic, powerGrid, radar } =
  ...
  const stream = {
    ...
    honestyNote: turnerAllowSynthetic()
      ...
      : 'Dynamic inputs are remote APIs only (NOAA SWPC, Open-Meteo soil, OpenWebRX IQ, HIFLD, geomagnetic). No synthetic soil placeholders. Fence gate strengths are uniform from live Kp / IQ only (no fabricated spatial phase). Registry baselines are static published inputs, not live animal measurements. Accuracy and capability improve significantly when tuned in collaboration with Turner teams on the ground (boundaries, fence truth, receiver placement, seasonal baselines).',
    ...
    pipeline: { ingest, scale, radar, synthesis, magnetic, powerGrid, fence, satellite, lstPass, steelPack },
    matrixRow: {
      at: synthesis.at,
      ingest,
      scale,
