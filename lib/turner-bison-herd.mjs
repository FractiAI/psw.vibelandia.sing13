...
export async function fetchRfHydrogenProxy() {
  ...
  try {
    const { frame, live, stale, captureError } = await getWebSdrFrame({ live: false });
...
export async function runTurnerBisonPipeline() {
  ...
  const { fence, satellite, magnetic, powerGrid, radar } = await runPassiveRadarFusion({
    wirePhaseUs,
    iqRms: rf.iqRms,
    kpLive,
    ingestAt,
  ...
  const stream = {
    ...
    honestyNote: turnerAllowSynthetic()
      ...
      : 'Dynamic inputs are remote APIs only (NOAA SWPC, Open-Meteo soil, OpenWebRX IQ, HIFLD, geomagnetic). No synthetic soil placeholders. Fence gate strengths are uniform from live Kp / IQ only (no fabricated spatial phase). Registry baselines are static published inputs, not live animal measurements.',
    ...
    pipeline: { ingest, scale, radar, synthesis, magnetic, powerGrid },
    matrixRow: {
      at: synthesis.at,
      ingest,
      scale,
