 ...
 * Multi-taxa wavefield: bison N-S keystone anchor (WP-GGM-MULTITAXA-UNGULATE-2026-06).
 */
...
export async function runTurnerBisonPipeline() {
  ...
  const { fence, satellite, lstPass, steelPack, magnetic, powerGrid, radar, laiPass, geography } =
  ...
  /** All public “sensor” extensions this build can reach — always merged into live stream (no env toggle). */
  let spaceBio;
  try {
    const { buildSpaceBioPanels } = await import('./turner-space-bio-panels.mjs');
    spaceBio = await buildSpaceBioPanels({
      geography,
      rf,
      fence,
      radar,
    ...
    spaceBio = {
      at: new Date().toISOString(),
      error: e?.message || 'space_bio_failed',
      channels: null,
      disclaimer: 'Space-bio pack failed during ingest; main fuse may still be valid — check logs.',
    };
  }
  ...
  let multiTaxaWavefield = null;
  try {
    const matrix = await loadMultiTaxaUngulateMatrix();
    multiTaxaWavefield = {
      docId: matrix.docId,
      bisonKeystone: bisonKeystoneAnchor(matrix),
      taxaCount: (matrix.taxa || []).length,
      honestyBoundary: matrix.honestyBoundary,
      whitepaper: matrix.whitepaper,
    };
  } catch {
    multiTaxaWavefield = { error: 'matrix_unavailable' };
  }

  const stream = {
    schema: 'turner-bison-stream/v3',
    ...
    honestyNote: turnerAllowSynthetic()
      ...
      : 'Dynamic inputs are remote APIs only (NOAA SWPC, Open-Meteo soil, OpenWebRX IQ, HIFLD, geomagnetic). No synthetic soil placeholders. Fence gate strengths are uniform from live Kp / IQ only (no fabricated spatial phase). Registry baselines are static published inputs, not live animal measurements. Default passive RF: internet OpenWebRX mapped to live fence coordinates — no on-premise Turner SDR required. On-premise receive-only SDR can upgrade RF fidelity when deployed. Space-bio ranch log is always attached: Open-Meteo blended column proxies at network centroid, Sentinel-2 L2A STAC catalog head over pasture bbox, plus RF/fuse reflectivity readouts — not CYGNSS DDM ingest and not GOES sounder L2 biology claims. Accuracy improves when tuned with Turner teams on the ground (boundaries, fence truth, seasonal baselines).',
    ...
    pipeline: { ingest, scale, radar, synthesis, magnetic, powerGrid, fence, satellite, lstPass, steelPack, laiPass, geography },
    matrixRow: {
      at: synthesis.at,
      ingest,
      scale,
