...
export function validateDateRange(startDate, endDate) {
  ...
  if (days > MAX_RANGE_DAYS) {
    return {
      ...
      message: `Maximum range is ${MAX_RANGE_DAYS} days (requested ${days}).`,
    };
  }
...
function buildSatelliteSnapshotForDay(soilByPastureDay, day, geography) {
...
/**
 * Full date-range pipeline: soil history → daily radar fuse → sampled head trajectories.
 */
export async function runTurnerBisonTimeseries({ startDate, endDate, sampleSize = SAMPLE_HEADS } = {}) {
  ...
  const [magnetic, powerGrid, soilPack, noaa, rf] = await Promise.all([
    fetchMagneticLayers(),
    fetchPowerGridLayer({
      networkBbox: mapBbox,
  ...
  const fence = fenceLineReturns(geography.pastures, wirePhaseUs, rf.iqRms, kpLive);

  const daily = [];

  for (const day of days) {
    const satellite = buildSatelliteSnapshotForDay(soilPack.byPastureDay, day, geography);
