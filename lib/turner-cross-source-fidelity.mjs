...
/** Individual-animal certainty stays bounded without animal-level sensors. */
...
function scorePastureCrossRef(pasture, ctx) {
  ...
  const pasturePlacementPct = Math.round((contributionSum * 100 + agreementMean * 22 + spatialStructure * 100) * 10) / 10;
...
export function computeCrossSourceFidelity(ctx) {
  ...
  let collarGradeProximityPct = Number(
    (meanPlacement * 0.72 + meanConfidence * 18 + Math.min(fusePct, 100) * 0.08).toFixed(1),
  );

  if (ctx.syntheticDataAllowed) {
    collarGradeProximityPct = Number((collarGradeProximityPct * 0.65).toFixed(1));
  }
  ...
  return {
    ...
    ceilings: {
      collarGradeWithoutCollarOrVhr: null,
    ...
    perPasture,
    honesty:
      'Collar-grade proximity = independent feeds agree (soil, LST, LAI, ET₀, SDR, fence geometry, magnetic, grid). It is NOT GPS collar position or per-animal identity. Value reflects current multi-source agreement (no fixed software cap).',
    note:
