 ...
 * Live GFZ Kp + public Movebank GPS collar trajectories (not Turner synthesis).
 */
...
export async function runRecentAnomalyModule(opts = {}) {
  const collar = await fetchPublicCollarDaily({
  ...
  const window = collar.window || {};
  const end = opts.endDate || window.analysisEnd || isoDate(new Date());
  const start90 = opts.start90 || window.analysisStart90 || addDays(end, -90);
  const start30 = addDays(end, -29);
  const start14 = addDays(end, -13);
  const baselineStart = addDays(start90, -365 * 5);

  const kpRows = await fetchGfzKp(baselineStart, end);
  ...
  return {
    ...
    range: { start90, start30, start14, end, baselineStart, collarWindow: window },
    dataHonesty: {
