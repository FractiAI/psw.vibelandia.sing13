/**
 * Upstash Redis REST — optional. Falls back to null when env unset (caller uses in-memory).
 */
export function upstashConfigured() {
  return !!(process.env.UPSTASH_REDIS_REST_URL && process.env.UPSTASH_REDIS_REST_TOKEN);
}

const DEFAULT_TIMEOUT_MS = 10_000;

function upstashTimeoutMs() {
  const n = Number(process.env.UPSTASH_TIMEOUT_MS);
  return Number.isFinite(n) && n > 0 ? n : DEFAULT_TIMEOUT_MS;
}

async function upstash(command, ...args) {
  const url = process.env.UPSTASH_REDIS_REST_URL;
  const token = process.env.UPSTASH_REDIS_REST_TOKEN;
  if (!url || !token) return null;

  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), upstashTimeoutMs());
  try {
    const res = await fetch(url, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json',
      },
      body: JSON.stringify([command, ...args]),
      signal: controller.signal,
    });
    if (!res.ok) return null;
    const data = await res.json();
    return data?.result ?? null;
  } catch {
    // Timeout/abort or transport error — treat as unavailable (callers fall back to in-memory / retry).
    return null;
  } finally {
    clearTimeout(timer);
  }
}
export async function redisSetJson(key, value, ttlSec) {
  const raw = JSON.stringify(value);
  const result = ttlSec
    ? await upstash('SET', key, raw, 'EX', String(ttlSec))
    : await upstash('SET', key, raw);
  return result === 'OK' || result === true;
}

export async function redisGetJson(key) {
  const raw = await upstash('GET', key);
  if (!raw) return null;
  try {
    return JSON.parse(raw);
  } catch {
    return null;
  }
}

export async function redisGet(key) {
  const raw = await upstash('GET', key);
  return raw ?? null;
}

export async function redisIncr(key) {
  const result = await upstash('INCR', key);
  if (result === null || result === undefined) return null;
  const n = Number(result);
  return Number.isFinite(n) ? n : null;
}

export async function redisLpush(key, value, maxLen = 500) {
  await upstash('LPUSH', key, value);
  await upstash('LTRIM', key, '0', String(maxLen - 1));
}

export async function redisLrange(key, start = 0, stop = -1) {
  const result = await upstash('LRANGE', key, String(start), String(stop));
  return Array.isArray(result) ? result : [];
}

export async function redisSadd(key, ...members) {
  if (!members.length) return 0;
  const result = await upstash('SADD', key, ...members);
  return result;
}

export async function redisSismember(key, member) {
  const result = await upstash('SISMEMBER', key, member);
  return result === 1 || result === true;
}
