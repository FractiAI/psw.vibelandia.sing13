/**
 * Cold-traffic visit funnel — Canvas → Concert program → Check In.
 * Check In page mirrors the pattern: Sound on → Check-in program → Creator Studio.
...
export function renderVisitGoldenPathHtml(active, opts = {}) {
  ...
  const doorsNote =
    active === 'reading-room'
      ? `<p class="visit-golden-path__doors-note">Scroll for the full paper menu by category — or <a href="/front-desk">Check In</a> when you are ready to board.</p>`
      : active === 'front-desk'
        ...
        : active === 'ship'
        ? `<p class="visit-golden-path__doors-note">You are on the ship board. Scroll for narrative, crew, and mission — or <a href="/front-desk">Check In</a>.</p>`
...
export function renderVisitGoldenPathHeroCtasHtml(active) {
  if (active === 'canvas') {
    return `<a class="btn btn--gold" href="/concierto-program">${PROGRAM_CTA_LABEL}</a>
        <a class="btn btn--gold" href="/front-desk">Check In</a>
        <a class="btn btn--ghost" href="#exhibit">Explore the exhibit</a>`;
  }
  if (active === 'front-desk') {
    return `<a class="btn btn-primary" href="/front-desk-program">${PROGRAM_CTA_LABEL}</a>
  ...
  return `<a class="btn btn-primary btn-player-lead" href="/concierto-program">${PROGRAM_CTA_LABEL}</a>
        <a class="btn btn-primary" href="/front-desk">Check In</a>
