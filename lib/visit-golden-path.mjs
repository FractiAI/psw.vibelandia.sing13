...
export function renderVisitGoldenPathHtml(active, opts = {}) {
  const receptionHref = opts.anchorReception ?? '/questfest#reception-lobby';
  const steps = [
    {
      id: 'exhibit',
      num: '1',
    ...
    },
    {
      id: 'reception',
      num: '3',
      label: active === 'reception' ? 'Registration · check-in' : 'Registration',
      href: receptionHref,
...
export function renderVisitGoldenPathHeroCtasHtml(active) {
  ...
  return `<a class="btn btn-primary btn-player-lead" href="/concierto-program">Concert program</a>
        <a class="btn btn-primary" href="/questfest#reception-lobby">Registration · check-in</a>
