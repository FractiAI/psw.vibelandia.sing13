...
export const VOYAGE_DECKS = [
  ...
  },
  {
    ...
    body: [
      ...
      'Landfalls connect here — Cartagena, Sierra, Tahoe, Downtown Reno as network nodes, not one harbor.',
...
export function renderVoyageMapDiagramHtml() {
  ...
  return `<section class="voyage-map-diagram" aria-labelledby="voyage-map-h">
      ...
      <p class="voyage-map-deck__tags"><strong>Network landfalls</strong> — nodes on the vessel, not one harbor:</p>
...
export function renderVoyageDirectoryIndexHtml() {
  ...
  const cabinCards = VOYAGE_CABINS.map(
    (c) =>
      `<li><a href="${voyageCabinHref(c.slug)}">${renderDirectoryThumb(c.image, c.imageAlt)}<span class="voyage-directory-card__text"><span class="sku">${c.skuDisplay}</span> <strong>${c.name}</strong><span>${c.serials.length} serial${c.serials.length === 1 ? '' : 's'}</span></span></a></li>`,
  ).join('\n        ');
...
<head>
  ...
  <meta name="description" content="Voyage map — deck stack, network landfalls, cabin SKUs, and serial registers for Players and NPCs." />
  <link rel="canonical" href="${canonical}" />
  <meta property="og:title" content="Voyage Map · SS Vibelandia" />
  <meta property="og:description" content="Holographic deck stack, landfalls, and cabin directory — brochure §14–22." />
...
<body class="voyage-flagship">
  <article class="wrap voyage-door-page">
    ...
    <header>
      <p class="kicker">Brochure §14–22 · Frontiersman Voyage · Voyage Map</p>
      <h1>Voyage Map</h1>
      <p class="dateline">${VOYAGE_DOOR_SPINE}</p>
    </header>
    <p class="lead">The map is the ship — deck stack from Core to Summit, network landfalls, and every cabin SKU door. <strong>Players</strong> chart the pattern; <strong>NPCs</strong> inhabit what you resource.</p>
