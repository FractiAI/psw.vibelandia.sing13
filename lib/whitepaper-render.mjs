...
import {
  GITHUB_MONOREPO_TREE,
  standaloneGithubHrefForResearch,
} from './standalone-suite-manifest.mjs';
...
export function normalizeRepoLinksInHtml(html) {
  return html.replace(/<a href="([^"]+)"/gi, (match, href) => {
    ...
    const standaloneHref = standaloneGithubHrefForResearch(href);
    if (standaloneHref) {
      return `<a href="${standaloneHref}"`;
    }
    ...
    // Fallback: monorepo tree for other research/ paths not yet published standalone
    const researchTree = standaloneGithubHrefForResearch(href) ||
      (() => {
        ...
        return `${GITHUB_MONOREPO_TREE}/research/${rest}${slash}`;
      })();
    if (researchTree && /research\//.test(href)) {
      return `<a href="${researchTree}"`;
    }
    return match;
  });
}
