...
import { DOC_ID, REGISTRY_ID, STUDY_TITLE, E_F, LAMBDA_EGS } from '../src/constants.mjs';
...
function mdReport(report) {
  const lines = [
    ...
    `| $E_F$ | ${E_F} |`,
    `| $\\lambda_{\\mathrm{EGS}}$ | ${LAMBDA_EGS} |`,
    '',
  ];
  for (const e of report.results.experiments) {
    lines.push(`### ${e.id} — ${e.title}`);
    lines.push(`- **Pass:** \`${e.pass}\``);
    if (e.interpretation) lines.push(`- ${e.interpretation}`);
...
async function main() {
  ...
  const report = {
    ...
    honestyBoundary:
      'DNA lattice holograph is architectural. Public T2T sizes + Euler–EGS math + agentic entropy. Biophoton 480 ms claims not executed here.',
    results,
  };
  ...
  console.log(
    JSON.stringify(
      {
        ok: results.all_pass,
        repo: 'https://github.com/FractiAI/synthobs-dna-lattice-holograph',
