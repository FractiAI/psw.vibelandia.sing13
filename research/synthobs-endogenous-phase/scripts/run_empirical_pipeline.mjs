#!/usr/bin/env node
/**
 * Empirical pipeline — Endogenous Conscious Intent Phase (TBME)
 * Doc: WP-SYNTHOBS-TBME-ENDOGENOUS-PHASE-2026-07-31
 */
...
function mdReport(report) {
  ...
  lines.push('');
  lines.push(
    'TBME architectural rubric. Validates score arithmetic, E_F protocol mnemonics, and clinical non-claim gate. Does **not** claim wet-lab magnetometry completion, clinical efficacy, or medical advice.',
  );
  lines.push('');
  return lines.join('\n');
}
...
async function main() {
  ...
  const report = {
    ...
    honestyBoundary:
      'TBME theoretical bio-medical & physical exploration. Architectural fixtures only — not clinical advice or mind-cure claims.',
    results,
  };
  ...
  console.log(
    JSON.stringify(
      {
        ok: results.all_pass,
        repo: 'https://github.com/FractiAI/synthobs-endogenous-phase',
