...
function mdReport(report) {
  ...
  lines.push(
    '## Honesty',
    '',
    'Catalog routing suite — awareness wormhole metaphor; not hardware teleport or clinical neuroscience proof.',
    '',
  );
  return lines.join('\n');
}
...
async function main() {
  ...
  const report = {
    ...
    honestyBoundary:
      'Human reality bridge validates Φ algebra and routing labels — not measured wormhole throats or DMN FLOPs.',
    results,
  };
  ...
  console.log(
    JSON.stringify(
      {
        ok: results.all_pass,
        repo: 'https://github.com/FractiAI/synthobs-human-omniversal-reality-bridge',
