...
function mdReport(report) {
  ...
  const lines = [
    ...
    '',
    `**Shells:** ${(f.gatewayShells || []).join(' · ')}`,
    `**Octave routing:** lattice fraction of flat **${(f.octaveRouting.latticeFractionOfFlat * 100).toFixed(2)}%** at N=${f.octaveRouting.nDomains} (reduction **${f.octaveRouting.reductionPctAtN.toFixed(1)}%**).`,
    `**Coherence:** after ${f.coherence.switches} switches — lattice **${f.coherence.latticeFinal.toFixed(2)}**, flat **${f.coherence.flatFinal.toFixed(2)}**.`,
    `**Companion receipt:** percentSaved=${f.companionReceipt.percentSaved ?? 'n/a'} (softPass=${f.companionReceipt.softPass}).`,
