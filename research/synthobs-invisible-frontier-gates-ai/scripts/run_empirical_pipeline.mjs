...
function mdReport(report) {
  ...
  lines.push(
    '## Honesty',
    '',
    'Voyage editorial catalog suite — not FLOP benchmarks, not prophecy, not displacement solved.',
    '',
  );
  return lines.join('\n');
}
...
async function main() {
  ...
  const report = {
    ...
    honestyBoundary:
      'Invisible Frontier validates editorial grammar and companion linkage — not policy verdicts or physics proofs.',
    results,
  };
  ...
  console.log(
    JSON.stringify(
      {
        ok: results.all_pass,
        repo: 'https://github.com/FractiAI/synthobs-invisible-frontier-gates-ai',
