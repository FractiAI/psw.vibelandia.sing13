...
function mdReport(report) {
  ...
  lines.push(
    '## Honesty',
    '',
    'Catalog / engine suite — not overturned MHD, not proven Coulomb bypass, not enrichment how-to, not nuclear-materials operations.',
    '',
  );
  return lines.join('\n');
}
...
async function main() {
  ...
  const report = {
    ...
    honestyBoundary:
      'Magneto-Harmonic Stellar validates Φ, actors, E_n sketch, protocols, dual-use refusal — not fusion proof.',
    results,
  };
  ...
  console.log(
    JSON.stringify(
      {
        ok: results.all_pass,
        repo: 'https://github.com/FractiAI/synthobs-magneto-harmonic-stellar-99-octave',
