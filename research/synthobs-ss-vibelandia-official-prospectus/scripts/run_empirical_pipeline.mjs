...
function mdReport(report) {
  const lines = [
    ...
    `| Φ_EGS | ${PHI_EGS} |`,
    `| Tempo | ${VESSEL_SPECS.tempoBpm} BPM |`,
    '',
  ];
  ...
  lines.push(
    '## Honesty',
    '',
    'Narrative foundation suite — not prophecy, not solar causation proofs, not medical tuning claims.',
    '',
  );
  return lines.join('\n');
}
...
async function main() {
  ...
  const report = {
    ...
    honestyBoundary:
      'Prospectus locks arc beats, vessel specs, doors — not predictive cosmology.',
    results,
  };
  ...
  console.log(
    JSON.stringify(
      {
        ok: results.all_pass,
        repo: 'https://github.com/FractiAI/synthobs-ss-vibelandia-official-prospectus',
