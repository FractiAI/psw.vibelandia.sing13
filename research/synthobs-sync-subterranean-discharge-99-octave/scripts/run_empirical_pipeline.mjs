...
function mdReport(report) {
  ...
  lines.push(
    '## Honesty',
    '',
    'Application lens suite — not multi-hazard prediction, not proven solar→quake/volcano causation.',
    '',
  );
  return lines.join('\n');
}
...
async function main() {
  ...
  const report = {
    ...
    honestyBoundary:
      'Sync subterranean discharge 99-octave application validates Φ, 8019 matrix, Colombia/Puracé fixtures — not operational forecasts.',
    results,
  };
  ...
  console.log(
    JSON.stringify(
      {
        ok: results.all_pass,
        repo: 'https://github.com/FractiAI/synthobs-sync-subterranean-discharge-99-octave',
