...
function mdReport(report) {
  ...
  lines.push(
    '## Honesty',
    '',
    'Catalog / engine suite — not multi-TeV benchtop proof, not CERN replacement, not unsupervised RF/pulsed-power authorization.',
    '',
  );
  return lines.join('\n');
}
...
async function main() {
  ...
  const report = {
    ...
    honestyBoundary:
      'Table-Top HEP validates Φ, bands, triad, p_eff sketch, protocols — not measured TeV equivalence.',
    results,
  };
  ...
  console.log(
    JSON.stringify(
      {
        ok: results.all_pass,
        repo: 'https://github.com/FractiAI/synthobs-table-top-hep-99-octave',
