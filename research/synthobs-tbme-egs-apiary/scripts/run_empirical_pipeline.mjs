...
function mdReport(report) {
  ...
  lines.push(
    '## Honesty boundary',
    '',
    'Catalog / Omni-Lattice lens arithmetic for Apiary–EGS coexistence metaphor. Does **not** claim ASI timelines, demography forecasts, or Core Part status.',
    '',
  );
  return lines.join('\n');
}
...
async function main() {
  ...
  const report = {
    ...
    honestyBoundary:
      'Apiary–EGS Omni-Lattice lens — catalog exploration only; not Core / Engine.',
    results,
  };
  ...
  console.log(
    JSON.stringify(
      {
        ok: results.all_pass,
        repo: 'https://github.com/FractiAI/synthobs-tbme-egs-apiary',
