/**
 * Higgs-Awareness Phase Coupling — deterministic suite.
...
export function experimentTriadicMatrix() {
  return {
    ...
    title: 'Triadic deceleration & matter matrix has 3 rows',
    n: TRIADIC_ROWS.length,
    rows: TRIADIC_ROWS,
    pass: TRIADIC_ROWS.length === 3,
    interpretation: 'QFT · pipe/magnet · Now catalog completeness.',
...
export function experimentPaperOnDisk() {
  ...
  if (monoOk) {
    ...
    hasPartVii = /Part VII/i.test(text);
  }
  return {
    id: 'E8_paper_on_disk',
    title: 'Canonical paper + Doc ID + Honesty + TBME + Part VII',
    monoOk,
    mirrorOk,
    hasDocId,
    hasHonesty,
    ...
    pass: monoOk && mirrorOk && hasDocId && hasHonesty && hasTbme && hasPartVii,
...
export function experimentScorecardOrder() {
  ...
  return {
    ...
    interpretation: 'Interpretive scorecard ordering for Part VII lens.',
