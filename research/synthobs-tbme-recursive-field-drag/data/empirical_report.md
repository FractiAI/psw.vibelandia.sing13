# The Recursive Field-Drag Theorem — Empirical Suite

**Document ID:** `WP-SYNTHOBS-TBME-RECURSIVE-FIELD-DRAG-2026-08-04`
**Registry ID:** `synthobs-tbme-recursive-field-drag-2026-08`
**Generated:** 2026-08-04T17:30:52.026Z

## Verdict

| Metric | Value |
|--------|-------|
| All experiments pass | `true` |
| Passed | 9 / 9 |
| E_F | 1.618033988749895 |

## Experiments

### E1_egs_phi — E_F = Φ_EGS fixture

- **Pass:** `true`
- **Interpretation:** Catalog harmonic key for field-drag phase contrast.
- **Honesty:** Architectural constant — not a replacement for μ₀.

```json
{
  "id": "E1_egs_phi",
  "title": "E_F = Φ_EGS fixture",
  "E_F": 1.618033988749895,
  "expected": 1.618033988749895,
  "pass": true,
  "interpretation": "Catalog harmonic key for field-drag phase contrast.",
  "honesty": "Architectural constant — not a replacement for μ₀."
}
```

### E2_ef_squared_identity — E_F² = E_F + 1

- **Pass:** `true`
- **Interpretation:** Golden-key identity used in terminal-velocity lock narrative.
- **Honesty:** Algebra of Φ — replayable fixture.

```json
{
  "id": "E2_ef_squared_identity",
  "title": "E_F² = E_F + 1",
  "lhs": 2.618033988749895,
  "rhs": 2.618033988749895,
  "pass": true,
  "interpretation": "Golden-key identity used in terminal-velocity lock narrative.",
  "honesty": "Algebra of Φ — replayable fixture."
}
```

### E3_z0_horizon — Free-space Z₀ ≈ 377 Ω fixture

- **Pass:** `true`
- **Interpretation:** Conductive horizon impedance narrative anchor.
- **Honesty:** SI free-space impedance constant — not a pipe resistivity measurement.

```json
{
  "id": "E3_z0_horizon",
  "title": "Free-space Z₀ ≈ 377 Ω fixture",
  "Z0": 376.730313668,
  "pass": true,
  "interpretation": "Conductive horizon impedance narrative anchor.",
  "honesty": "SI free-space impedance constant — not a pipe resistivity measurement."
}
```

### E4_lenz_denominator — Classical Lenz factor denominator σ B₀² ℓ² finite & positive

- **Pass:** `true`
- **Interpretation:** Model shares classical structure; E_F² scales it (E5).
- **Honesty:** Toy numbers — not a lab calibration.

```json
{
  "id": "E4_lenz_denominator",
  "title": "Classical Lenz factor denominator σ B₀² ℓ² finite & positive",
  "den": 5960,
  "pass": true,
  "interpretation": "Model shares classical structure; E_F² scales it (E5).",
  "honesty": "Toy numbers — not a lab calibration."
}
```

### E5_terminal_ef2_scale — v_terminal model scales classical by 1/E_F²

- **Pass:** `true`
- **Interpretation:** Part V invariant arithmetic on the design equation.
- **Honesty:** Closed-form model — not measured magnet-pipe terminal speed.

```json
{
  "id": "E5_terminal_ef2_scale",
  "title": "v_terminal model scales classical by 1/E_F²",
  "classical": 0.00008229865771812082,
  "lattice": 0.00003143529001982829,
  "ratio": 2.618033988749895,
  "pass": true,
  "interpretation": "Part V invariant arithmetic on the design equation.",
  "honesty": "Closed-form model — not measured magnet-pipe terminal speed."
}
```

### E6_asqueezed_energy_density — ‖A‖²/(2μ₀) energy-density form positive

- **Pass:** `true`
- **Interpretation:** Brake-force narrative uses magnetic energy density gradient.
- **Honesty:** Algebraic positivity — not a force measurement.

```json
{
  "id": "E6_asqueezed_energy_density",
  "title": "‖A‖²/(2μ₀) energy-density form positive",
  "A": 0.0012,
  "u": 0.5729577951308231,
  "MU_0": 0.0000012566370614359173,
  "pass": true,
  "interpretation": "Brake-force narrative uses magnetic energy density gradient.",
  "honesty": "Algebraic positivity — not a force measurement."
}
```

### E7_epistemic_map_rows — Epistemic twin table has five rows

- **Pass:** `true`
- **Interpretation:** Catalog completeness for Now ↔ field-drag map.
- **Honesty:** Structural table — not clinical evidence.

```json
{
  "id": "E7_epistemic_map_rows",
  "title": "Epistemic twin table has five rows",
  "n": 5,
  "rows": [
    [
      "Falling magnet",
      "Unbounded raw field potential / awareness"
    ],
    [
      "Conductive boundary wall",
      "Biological body & physical Earth matrix"
    ],
    [
      "Recursive field coupling",
      "Self-referential awareness"
    ],
    [
      "Terminal velocity drag",
      "Temporal deceleration into the Now"
    ],
    [
      "Dissipated electric heat",
      "Actions, thoughts & mental artifacts"
    ]
  ],
  "pass": true,
  "interpretation": "Catalog completeness for Now ↔ field-drag map.",
  "honesty": "Structural table — not clinical evidence."
}
```

### E8_paper_on_disk — Canonical paper + Doc ID + Honesty + TBME disclaimer

- **Pass:** `true`
- **Interpretation:** Catalog fidelity for Omni-Lattice appendix sync.
- **Honesty:** Filesystem receipt.

```json
{
  "id": "E8_paper_on_disk",
  "title": "Canonical paper + Doc ID + Honesty + TBME disclaimer",
  "monoOk": true,
  "mirrorOk": true,
  "hasDocId": true,
  "hasHonesty": true,
  "hasTbme": true,
  "registryId": "synthobs-tbme-recursive-field-drag-2026-08",
  "pass": true,
  "interpretation": "Catalog fidelity for Omni-Lattice appendix sync.",
  "honesty": "Filesystem receipt."
}
```

### E9_scorecard_order — Field-Drag rubric overall > Classical overall

- **Pass:** `true`
- **Interpretation:** Interpretive scorecard ordering for Part V lens.
- **Honesty:** Rubric arithmetic — not SI accuracy of nature.

```json
{
  "id": "E9_scorecard_order",
  "title": "Field-Drag rubric overall > Classical overall",
  "classicalOverall": 71,
  "dragOverall": 96.75,
  "fixtures": {
    "CLASSICAL": {
      "coherence": 74,
      "irreducibility": 68,
      "overall": 71
    },
    "FIELD_DRAG": {
      "coherence": 97.5,
      "irreducibility": 96,
      "overall": 96.75
    }
  },
  "pass": true,
  "interpretation": "Interpretive scorecard ordering for Part V lens.",
  "honesty": "Rubric arithmetic — not SI accuracy of nature."
}
```

## Honesty boundary

Catalog / model arithmetic for the Recursive Field-Drag Omni-Lattice lens. Does **not** claim SI Lenz overthrow, calibrated magnet-pipe invoices, or clinical proof of the Now.
