...
function mdReport(report) {
  const lines = [
    ...
    `| Φ_EGS | ${PHI_EGS} |`,
    `| 9×81 | ${MATRIX_9x81} |`,
    `| Octaves | ${OCTAVE_SEGMENTS} |`,
    `| 99×81 companion | ${HOLOGRAPHIC_KEY_DIGITS} |`,
    '',
  ];
  ...
  lines.push(
    '## Honesty',
    '',
    'Catalog / engine suite — not multi-domain causation proof, not operational hazard or clinical tooling.',
    '',
  );
  return lines.join('\n');
}
...
async function main() {
  ...
  const report = {
    ...
    honestyBoundary:
      'Tensor decoupling 99-octave Omni-Lattice validates Φ, 9×81 matrix, 11-tier ladder, Colombia fixtures — not physics unification or forecasts.',
    results,
  };
  ...
  console.log(
    JSON.stringify(
      {
        ok: results.all_pass,
        repo: 'https://github.com/FractiAI/synthobs-tensor-decoupling-99-octave-omni-lattice',
