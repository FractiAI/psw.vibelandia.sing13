...
import {
  ...
  CATALOG_LAYER_SANDBOX_FUNCTIONS,
...
describe('AI catalog layer · Infinite Octaves Omniversal Lattice Chat', () => {
  ...
  it('defines five catalog-layer innovations with unique deliverables', () => {
    ...
    expect(CATALOG_LAYER_SECURITY_FABRIC.id).toBe('security-fabric');
    expect(CATALOG_LAYER_SECURITY_FABRIC.delivers.length).toBeGreaterThanOrEqual(4);
    expect(CATALOG_LAYER_SANDBOX_FUNCTIONS.length).toBeGreaterThanOrEqual(5);
  ...
  it('features five innovations + security fabric on catalog-layer guest surfaces', () => {
    for (const rel of [
      ...
      expect(html, `${rel} security fabric`).toContain('security-fabric');
      expect(html, `${rel} without-it claim`).toContain('Cannot achieve without it');
    }
  });

  it('ships an interactive with/without executive sandbox on EGS briefing', () => {
    const briefing = read('interfaces/infinite-octave-egs-catalog-briefing-portals.html');
    expect(briefing).toContain('id="egsb-sandbox"');
    expect(briefing).toContain('data-mode');
    expect(briefing).toContain('Without catalog layer');
    expect(briefing).toContain('With catalog layer');
    expect(briefing).toContain('Board brief latency');
    expect(briefing).toContain('Security posture');
    expect(briefing).toContain('Goldilocks Net Zero');
  });
});
