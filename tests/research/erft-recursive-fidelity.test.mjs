...
describe('ERFT V3 nest-grammar recursive fidelity', () => {
  it('uses V3 protocol id', () => {
    expect(PROTOCOL_VERSION).toMatch(/^ERFT-V3-/);
  });
  ...
  it('empirical suite passes with nest grammar (not V2 sin geometry)', async () => {
    ...
    expect(e1?.scoreboard?.lowest_mean_rfd_arm).toBe('egs');
    const mean = e1.scoreboard.mean_final_rfd_by_arm;
    const improvement =
      (mean.baseline - mean.egs) / mean.baseline;
  ...
  it('committed empirical report matches live V3 pipeline', async () => {
    ...
    expect(onDisk.protocol).toMatch(/^ERFT-V3-/);
